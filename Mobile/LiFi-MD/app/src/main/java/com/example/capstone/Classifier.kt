...
class Classifier(assetManager: AssetManager, modelPath: String, labelPath: String, inputSize: Int) {
    ...
    data class Recognition(
        ...
        companion object CREATOR : Parcelable.Creator<Recognition> {
            override fun createFromParcel(parcel: Parcel): Recognition {
                return Recognition(parcel)
            }

            override fun newArray(size: Int): Array<Recognition?> {
...
class ClassifierSigmoid(assetManager: AssetManager, modelPath: String, labelPath: String, inputSize: Int) {
    private var interpreter: Interpreter
    private var lableList: List<String>
    private val INPUT_SIZE: Int = inputSize
    private val PIXEL_SIZE: Int = 3
    ...
    private fun getSortedResult(labelProbArray: Array<FloatArray>): List<ClassifierSigmoid.Recognition> {
        Log.d("Classifier", "List Size:(%d, %d, %d)".format(labelProbArray.size, labelProbArray[0].size, lableList.size))

        val pq = PriorityQueue(
            MAX_RESULTS,
            Comparator<ClassifierSigmoid.Recognition> { (_, _, confidence1), (_, _, confidence2) ->
                java.lang.Float.compare(confidence1, confidence2) * -1
            })

        val confidence = labelProbArray[0][0]
        if (confidence >= THRESHOLD) {
            pq.add(ClassifierSigmoid.Recognition("" + 1, lableList[1], confidence))
        } else {
            pq.add(ClassifierSigmoid.Recognition("" + 0, lableList[0], confidence))
        }
        Log.d("Classifier", "pqsize:(%d)".format(pq.size))

        val recognitions = ArrayList<ClassifierSigmoid.Recognition>()
        val recognitionsSize = Math.min(pq.size, MAX_RESULTS)
        for (i in 0 until recognitionsSize) {
            recognitions.add(pq.poll())
        }
